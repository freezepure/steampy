...
class LoginExecutor:
    ...
    def _api_call(self, method: str, service: str, endpoint: str, version: str = 'v1', params: dict = None) -> Response:
        ...
        if method.upper() == 'GET':
            return self.session.get(url, params=params, headers=headers,proxies = {"http":"127.0.0.1:1081","https":"127.0.0.1:1081"})
        elif method.upper() == 'POST':
            return self.session.post(url, data=params, headers=headers,proxies = {"http":"127.0.0.1:1081","https":"127.0.0.1:1081"})
        else:
    ...
    def _fetch_rsa_params(self, current_number_of_repetitions: int = 0) -> dict:
        # self.session.post(SteamUrl.COMMUNITY_URL,proxies = {"http":"127.0.0.1:1081","https":"127.0.0.1:1081"})
        self.session.get(SteamUrl.COMMUNITY_URL,proxies = {"http":"127.0.0.1:1081","https":"127.0.0.1:1081"})
    ...
    def _perform_redirects(self, response_dict: dict) -> None:
        ...
        for pass_data in parameters:
            ...
            self.session.post(pass_data['url'], pass_data['params'],proxies = {"http":"127.0.0.1:1081","https":"127.0.0.1:1081"})
    ...
    #     response = self.session.post(SteamUrl.LOGIN_URL + '/jwt/finalizelogin', data=finalized_data,proxies = {"http":"127.0.0.1:1081","https":"127.0.0.1:1081"})
    #     return response
    def _finalize_login(self) -> Response:
        sessionid = self.session.cookies['sessionid']
        redir = f'{SteamUrl.COMMUNITY_URL}/login/home/?goto='
        ...
        return self.session.post("https://login.steampowered.com/jwt/finalizelogin", headers = headers, files = files,proxies = {"http":"127.0.0.1:1081","https":"127.0.0.1:1081"})
